...
# Fungsi untuk mengelompokkan nama barang
...
def preprocess_data(data):
    ...
    transaksiSJYEncoded = transaksiSJY.applymap(lambda x: 1 if x >= 1 else 0)
    transaksiSJY = transaksiSJYEncoded
    transaksiSJY.fillna(0, inplace=True)
...
def plot_data_preprocessing(filtered_data):
    ...
    # Visualisasi grafik untuk jumlah transaksi per BARANG (hanya 10 data teratas)
    ...
    # Visualisasi line chart untuk jumlah barang yang ditransaksikan per DEBIT (hanya 10 data teratas)
    ...
    if selected == "Filter":
        if st.session_state.data is not None:
            ...
            if start_date <= end_date:
                ...
                # Run Apriori algorithm based on user input
                frq_items = apriori(preprocessed_data,
                                    min_support=min_support, use_colnames=True)

                # Collect inferred rules into a dataframe
